					...
					<div class="col-lg-12">
						<div class="shop__cart__table">
							<table>
								...
								<tbody>
									<c:forEach var="list" items="${basketList}" varStatus="index">
										<tr>
											<td class="cart__product__item"><img
												src="${list.pImg1}" alt="상품이미지">
												<div class="cart__product__item__title">
													<h6><a href="getProduct.do?pNum=${list.pNum}">${list.pName}</a></h6>
												</div></td>
											<td class="cart__price">${list.pPrice} 원</td>
											<td class="cart__quantity">
												<form action="basketUpdate.do" method="post">
													<input type="hidden" name="index" value="${index.index}">
														<input type="number" class="pro-qty" name="hAmount" value="${list.hAmount}" min="1">
													<button type="submit" class="site-btn">수정하기</button>
												</form>
											</td>
											<td class="cart__total">${list.pPrice * list.hAmount} 원</td>
											<td class="cart__close"><a href="basketDelete.do?index=${index.index}"><span class="icon_close"></span></a></td>
										</tr>
				...
				<div class="col-lg-4 offset-lg-2">
					<div class="cart__total__procced">
						<c:if test="${!empty basketList}">
							...
							<a href="pay.jsp" class="primary-btn">결제</a>
						</c:if>
						<c:if test="${empty basketList}">
							<h6>물품을 담아주세요!</h6>
						</c:if>
